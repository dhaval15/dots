...
function revelation.expose_callback(t, zt, clientlist)
    ...
    capi.keygrabber.run(function (mod, key, event) 
        ...
        if key == "ISO_Level3_Shift" or key == "Escape" then
...
function revelation.init(args)
    ...
    for i = 1, #revelation.charorder do
        local char = revelation.charorder:sub(i,i)
        hintbox[char] = wibox({
          fg = args.fg or revelation.fg,
          bg = args.bg or revelation.bg,
          border_color = args.border_color or revelation.border_color,
          border_width = args.border_width or revelation.border_width
        })
